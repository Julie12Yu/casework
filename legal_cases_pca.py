...
def create_interactive_visualization(embedding, categories, titles, summaries, output_file='legal_cases_pca_interactive.html'):
    """Create interactive PCA visualization with correct color mapping"""
    ...
    fig.update_layout(
        title={
            'text': 'Interactive Legal Cases PCA Visualization',
            'x': 0.5,
            'font': {'size': 24}
        },
...
def main():
    """Main function to run the complete interactive PCA visualization pipeline"""
    ...
    embedding, pca_model = perform_pca(features)
...
if __name__ == "__main__":    
    try:
        main()
        print("Interactive PCA visualization completed successfully!")
